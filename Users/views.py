...
def get_user_data(token_data):
    ...
    """
    url = "https://www.googleapis.com/userinfo/v2/me?access_token=" + token_data.get(
        "access_token"
    )
    ...
    try:
        with urllib.request.urlopen(url) as response:
            if response.status != 200:
                raise RuntimeError(
                    "Request error: ", response.status, response.read().decode()
                )
...
def sign_up_with_google(request):
    ctx = {"redirect_uri": GOOGLE_OAUTH_URI}
...
class LoginView(auth_views.LoginView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({"google_oauth_uri": GOOGLE_OAUTH_URI})
        return context
